...
module.exports = (client) => {
    ...
    client.distube
                ...
                embeds: [
                  new MessageEmbed()
                    ...
                    .setTitle(
                      `${
                        newQueue.autoplay
                          ? `${client.allEmojis.check_mark} **Enabled Autoplay**`
                          : `${client.allEmojis.x} **Disabled Autoplay**`
                      }`
                    )
                    .setFooter(
                      `💢 Action by: ${member.user.tag}`,
              ...
              i.reply({
                embeds: [
                  new MessageEmbed()
                    ...
                    .setTitle(
                      `${
                        newQueue.repeatMode == 1
                          ? `${client.allEmojis.check_mark} **Enabled Song-Loop**`
                          : `${client.allEmojis.x} **Disabled Song-Loop**`
                      }`
                    )
                    .setFooter(
                      `💢 Action by: ${member.user.tag}`,
                    ...
                    .setTitle(
                      `${
                        newQueue.repeatMode == 2
                          ? `${client.allEmojis.check_mark} **Enabled Queue-Loop**`
                          : `${client.allEmojis.x} **Disabled Queue-Loop**`
                      }`
                    )
                    .setFooter(
                      `💢 Action by: ${member.user.tag}`,
        ...
        queue.textChannel.send({
          embeds: [
            new MessageEmbed()
              ...
              .addField(
                `⌛ **Estimated Time:**`,
                `\`${queue.songs.length - 1} song${queue.songs.length > 0 ? "s" : ""}\` - \`${(
        ...
        queue.textChannel.send({
          embeds: [
            new MessageEmbed()
              ...
              .setDescription(
                `👍 Playlist: [\`${playlist.name}\`](${playlist.url ? playlist.url : ""})  -  \`${
                  playlist.songs.length
                } Song${playlist.songs.length > 0 ? "s" : ""}\``
              )
              .addField(
                `⌛ **Estimated Time:**`,
                `\`${queue.songs.length - -playlist.songs.length} song${
                  queue.songs.length > 0 ? "s" : ""
                }\` - \`${(Math.floor(((queue.duration - playlist.duration) / 60) * 100) / 100)
                  .toString()
                  .replace(".", ":")}\``
              )
              .addField(`🌀 **Queue Duration:**`, `\`${queue.formattedDuration}\``),
      ...
      .on(`finishSong`, (queue, song) => {
        var embed = new MessageEmbed()
          ...
          .setAuthor(
            `${song.name}`,
            "https://cdn.discordapp.com/attachments/883978730261860383/883978741892649000/847032838998196234.png",
            song.url
          )
          .setDescription(
            `See the [Queue on the **DASHBOARD** Live!](${
  ...
  function receiveQueueData(newQueue, newTrack) {
    ...
    var embed = new MessageEmbed()
      ...
      .addField(
        `🎧 DJ-Role${client.settings.get(newQueue.id, "djroles").length > 1 ? "s" : ""}:`,
        `>>> ${djs}`,
        client.settings.get(newQueue.id, "djroles").length > 1 ? false : true
      )
      ...
      .addField(
        `↪️ Autoplay:`,
        `>>> ${newQueue.autoplay ? `${client.allEmojis.check_mark}` : `${client.allEmojis.x}`}\n\n`,
        true
      )
      ...
      .setFooter(
        ` ${newTrack.user.tag}`,
        newTrack.user.displayAvatarURL({
          dynamic: true,
        })
      );
    ...
    // const row = new MessageActionRow().addComponents([skip, stop, pause, autoplay, shuffle]);// Gốc
    const row = new MessageActionRow().addComponents([skip, stop, pause, autoplay]);
    // const row2 = new MessageActionRow().addComponents([songloop, queueloop, forward, rewind, lyrics]); //Gốc
    const row2 = new MessageActionRow().addComponents([shuffle, songloop, queueloop]);
    return {
      embeds: [embed],
      components: [row, row2],
    };
